...
namespace ReservationSystemWebAPI.Tests.Services
{
    public class UserServiceTests
    {
        ...
        public async Task UpdateAsync_ConcurrencyConflict_ThrowsException()
        {
            ...
            await Assert.ThrowsAsync<DbUpdateConcurrencyException>(() => _service.UpdateAsync(user));
        }
        ...
        public async Task DeleteAsync_ConcurrencyConflict_ThrowsException()
        {
            int id = 6;
            _mockRepo.Setup(r => r.ExistsAsync(id)).ReturnsAsync(true);
            _mockRepo.Setup(r => r.DeleteAsync(id)).ThrowsAsync(new DbUpdateConcurrencyException());

            await Assert.ThrowsAsync<DbUpdateConcurrencyException>(() => _service.DeleteAsync(id));
        }
